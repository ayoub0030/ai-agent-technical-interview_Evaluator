...
const ELEVEN_LABS_API_KEY = import.meta.env.VITE_ELEVEN_API_KEY

if (!ELEVEN_LABS_API_KEY) {
  console.warn('[Grading] Warning: VITE_ELEVEN_API_KEY not configured. Transcript fetching will fail.')
}
...
/**
 * Calls Next.js backend API to grade the interview using Gemini
 */
export async function gradeInterview(input: GradingInput): Promise<GradingOutput> {
  console.log('[Grading] Calling backend API to grade interview')

  try {
    // Try Next.js backend first (more reliable)
    // Frontend runs on 5173, backend runs on 3000
    const backendUrl = 'http://localhost:3000/api/grade-interview'
    console.log('[Grading] Calling backend at:', backendUrl)
    ...
    const data = await response.json()
    console.log('[Grading] Interview graded successfully via backend API')
    return data as GradingOutput
  } catch (backendError) {
    console.warn('[Grading] Backend API failed, trying Supabase Edge Function:', backendError)
    ...
    try {
      ...
      console.log('[Grading] Interview graded successfully via Edge Function')
      return data as GradingOutput
    } catch (edgeError) {
      console.error('[Grading] Both grading methods failed:', backendError, edgeError)
      throw new Error(`Failed to grade interview: ${edgeError}`)
    }
  }
}
...
export async function saveGradingResults(
  ...
  console.log('[Grading] Saving results to database for assessment:', assessmentId)
  ...
  const { error } = await supabase
    ...
    .insert({
      ...
      tradeoff_analysis: results.scores.trade_off_analysis,
...
export async function orchestrateGrading(
  ...
  try {
    ...
    await saveGradingResults(results, transcript, normalizedDiagram, assessmentId)
