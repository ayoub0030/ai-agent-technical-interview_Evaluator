...
export function useDiagramElevenSync(opts: {
  ...
  const check = useCallback(() => {
    ...
    if (diff1HasChange && !diff2HasChange && currentStateRef.current !== "") {
      if (!sendContextualUpdate) {
        console.warn('[DiagramSync] sendContextualUpdate is NOT available - cannot send diagram to agent');
        return;
      }

      console.log('[DiagramSync] Changes detected then stabilized - sending update to agent');
      ...
      console.log('[DiagramSync] Sending diagram update, size:', text.length, 'bytes');
      sendContextualUpdate(text);
      console.log('[DiagramSync] Diagram update sent successfully');
    }
  ...
  return {
    ...
    pushNow: () => {
      if (!sendContextualUpdate) {
        console.warn('[DiagramSync] pushNow called but sendContextualUpdate not available');
        return;
      }
      ...
      console.log('[DiagramSync] pushNow: Sending diagram, size:', text.length, 'bytes');
      sendContextualUpdate(text);
      console.log('[DiagramSync] pushNow: Diagram sent');
    }
  };
}
